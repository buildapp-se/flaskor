...
export function CellarTable({ rows, query, sort, dir, onSort }: { rows: Drink[]; query: string; sort: SortKey; dir: SortDir; onSort: (key: SortKey) => void }) {
    ...
    <div className="fl-tableview">
      ...
      <div className="fl-card fl-tablewrap">
        <table className="fl-table">
          ...
          <tbody>
            {rows.map((d) => (
              <tr key={d.id} className={d.count === 0 ? 'fl-table__row fl-table__row--muted' : 'fl-table__row'} onClick={(e) => !(e.target as HTMLElement).closest('a') && navigate(detailPath(d.id))}>
                {shown.map((c) => (
