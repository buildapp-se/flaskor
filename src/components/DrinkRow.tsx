...
export function DrinkRow({ drink, actions, muted = false }: { drink: Drink; actions?: ReactNode; muted?: boolean }) {
  ...
  return (
    <div className={muted ? 'fl-row fl-row--muted' : 'fl-row'} onClick={open} role="link" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && navigate(detailPath(drink.id))}>
      ...
      <div className="fl-row__main">
        ...
        {meta && <div className="fl-row__meta fl-desktop-only">{meta}</div>}
      ...
      <div className="fl-row__price">{drink.price_paid !== null ? kr(drink.price_paid) : drink.price_current !== null ? kr(drink.price_current) : ''}</div>
