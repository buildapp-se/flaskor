...
- [ ] `[P1]` **Patrik kör rotationen** (klassificeraren stoppar hemlighetsskrivning 2026-09-16, kommandot står i HANDOFF): byt grindkoden (`.dev.vars`, `wrangler secret bulk`, Actions-hemligheten `FLASKOR_GATE_CODE`). Den har legat i två webbläsare och ger fortfarande hushåll 1.
...
- [x] `[P3]` (rättade 2026-09-16, `7727340`, `c37dd79`, `32e7505`; **kvar:** Firebase SDK 11.6.1 mot 12.19, CDN-bytet i `src/auth.ts` kräver ett inloggningstest i webbläsare) OWASP, låga: 500-svar ekar `error.message` (`index.ts`); `sb_product_id` interpoleras rått i `stockUrl` (`stock.ts`), kräv `^\d+$`; CSV-export citerar inte inledande `= + - @` (`src/export.ts`); Caviste-hämtning utan `https:`-krav och med följda redirects (`caviste.ts`); localhost-origins i prod `FRONTEND_ORIGINS`; wrangler 4.129 (fix i 4.131) och Firebase SDK 11.6.1 mot 12.19.
