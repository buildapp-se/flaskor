...
| Grindkod | `gate code` | Före inloggningen den delade koden till appen. Nu tjänsteåtkomst till hushåll 1 (nattskript, skript) och engångsvägen in i hushåll 1 för ett konto (beslut 2, 10) |
...
- **Åtkomst** (beslut 2, byggd 2026-09-15): Firebase Auth i projektet `flaskor-d3762`, Google eller e-post med lösenord, SDK:n laddad från Googles CDN som i Beefcake. Klienten skickar ID-token som `Authorization: Bearer`; Workern verifierar den mot Googles nycklar (`jose`), kräver bekräftad e-post och slår upp hushållet i `member`. Grindkoden jämförs först och ger tjänsteåtkomst till hushåll 1; `POST /api/refresh-all` och `POST /api/assortment` kräver den och nekar konton. Inloggningen slås på med `apiKey` i `src/config.ts`; tom nyckel ger den gamla grinden. En webbläsare med sparad grindkod flyttar kontot till hushåll 1 vid första inloggningen, utan klick.
