...
- **Spegeln av sortimentet** (2026-09-12, migrering 0006, beslut 23): tabellen `sb_product` bär hela Systembolagssortimentet (27 035 rader) som vår `Product`-JSON per artikelnummer plus en söktext, och `sb_meta` bär `imported_at`. Fylls varje natt av GitHub Actions (`.github/workflows/assortment.yml` kl. 01:30 UTC): `scripts/assortment.ts` laddar tredjepartsdumpen, skalar raderna till fälten Workern läser (`shared/assortment.ts`) och postar 300 åt gången till `POST /api/assortment`, sist `done` som tar bort rader från äldre körningar och stämplar spegeln. **Workern läser inte dumpen själv**: 100 MB parsning kostade 2 s CPU för 9 000 rader och dog på Cloudflares fel 1102 (kontot är på gratisplanen). Spegeln är reserv för söket och för produktuppslag (nedan), och källa för natten. Den saknar lagersaldo per butik.
