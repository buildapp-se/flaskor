...
| Hushåll | `household` | Ägaren av all data (beslut 22). Varje konto får ett eget vid första inloggningen; hushåll 1 är Patrik och Julias |
| Konto | `member` | En Firebase-inloggning (Google eller bekräftad e-post) knuten till ett hushåll via uid (beslut 2, 2026-09-15) |
| Inbjudningskod | `invite_code` | Tio hextecken per hushåll; den som skriver in den går med, om det egna hushållet är tomt |
| Tjänstenyckel | `SERVICE_TOKEN` | Hette grindkoden (`GATE_CODE`) till 2026-09-16: före inloggningen den delade koden till appen. Nu bara tjänsteåtkomst till hushåll 1 (nattjobbet i Actions som `FLASKOR_SERVICE_TOKEN`, lokala skript); vägen in i hushåll 1 för ett konto togs bort 2026-09-16 när ägarna var inloggade (beslut 2, 10) |
...
- `price_paid` (per flaska, kr), `price_current` (senast kända pris i källan), `price_checked_at`, `availability`: `in_stock` | `temporarily_out` (centrallagret) | `supplier_out` (ordervara, leverantören) | `sold_out` (kommer inte tillbaka) | `discontinued` | `unknown` (beslut 23, vidgad 2026-09-13), `sb_assortment`: Systembolagets sortimentskod (BS ordervara, FS fast, TSE/TSV/TST tillfälligt, TSLS lokalt, TSS säsong, FSÖ webblansering), så Önskelistan kan säga "Ordervara · Slut hos leverantören"
...
- **Drucken-logg** (2026-09-09, beslut 16, migrering 0005): egen tabell `tasting` med `drink_id`, `drunk_on` (datum, inte tidsstämpel), `rating` (heltal 1 till 5, nullbart) och `note`. En rad kan drickas många gånger, därför en tabell och inte fler kolumner på `drink`. Loggen tas bort med raden. **"Drack en" skriver inte i loggen**: beslut 16 säger uttryckligen ingen ruta och inget betyg vid nedräkningen, så anteckningen görs för hand i detaljvyn, gärna i efterhand. `GET /api/drinks` bär tre härledda fält ur tabellen, `last_drunk_on`, `last_rating` och `tasting_count`, så listan kan visa senaste avsmakningen utan ett anrop per rad. De är läsfält: de finns inte på `drink`, går inte att skriva, och räknas fram i listfrågan.
...
**Hushåll och konton** (2026-09-15, migrering 0008): `household` har `name` och `invite_code`, `member` har `uid` (Firebase, primärnyckel), `household_id` och `email`. Ett konto hör till exakt ett hushåll. Byte nekas med 409 när det egna hushållet har rader, och ett hushåll utan medlemmar raderas med rader och loggar (utom hushåll 1, som grindkoden pekar på). Radera konto tar medlemskapet, och hushållet om ingen annan är kvar.

Ingen plats (beslut 9).
...
- **Nattlig uppdatering** (beslut 23): cron i Workern uppdaterar pris, årgång och tillgänglighet för varje artikelnummer en gång per natt (dedupe över hushåll). Sedan 2026-09-12 läses värdena ur **spegeln** (nedan) när den är yngre än 36 timmar; bara nummer spegeln saknar hämtas från produktsidan, och taket på 50 gäller bara dem. En "uppdatera"-knapp per rad gör samma sak på begäran, med produktsidan först och spegeln som reserv.
- **Spegeln av sortimentet** (2026-09-12, migrering 0006, beslut 23): tabellen `sb_product` bär hela Systembolagssortimentet (27 035 rader) som vår `Product`-JSON per artikelnummer plus en söktext, och `sb_meta` bär `imported_at`. Fylls varje natt av GitHub Actions (`.github/workflows/assortment.yml` kl. 01:30 UTC): `scripts/assortment.ts` laddar tredjepartsdumpen, skalar raderna till fälten Workern läser (`shared/assortment.ts`) och postar 300 åt gången till `POST /api/assortment`, sist `done` med alla artikelnummer, som tar bort rader utanför listan och stämplar spegeln. **Bara rader vars json ändrats skrivs** (`ON CONFLICT DO UPDATE ... WHERE`, 2026-09-13): D1 räknar varje skriven rad mot kontots dagskvot på 100 000, och `INSERT OR REPLACE` kostade två per rad, 54 000 per natt; därför säger `updated_at` när raden senast ändrades, inte när den senast sågs. **Workern läser inte dumpen själv**: 100 MB parsning kostade 2 s CPU för 9 000 rader och dog på Cloudflares fel 1102 (kontot är på gratisplanen). Spegeln är reserv för söket och för produktuppslag (nedan), och källa för natten. Den saknar lagersaldo per butik.
...
- **Caviste-import via produktlänk** (2026-09-09, beslut 6): `GET /api/caviste?q=<länk>` läser lådans produktsida och ger ett förhandsvisningsobjekt per vin i den. Sidan bär hela raden i en tabell längst ner, så antal, årgång, namn, pris, typ, ursprung, druvor, alkohol, drickfönster, serveringstemperatur, karaffering, smaknot och matförslag kommer med. Bilden tas ur radens egen cell, alltså rätt flaska utan gissning. Sidformatet varierar mellan lådor (CAV0143 har en extra `<em>` runt faktarutan, CAV0179 inte), så fälten plockas ur hela specen och inte ur en viss tagg. "Direkt till källaren" sparar lådans antal.
- **Sök på namn** (2026-09-09): `GET /api/search?q=` kör Systembolagets sök med samma publika frontendnyckel som skanningen och ger kandidatlistan rakt av, utan omrankning: användaren skrev frågan själv, så sökmotorns ordning är den bästa gissningen. I Lägg till delar namnet ruta med artikelnummer, streckkod och länkar. Rena bokstäver går direkt till söket; en fråga med siffror provar artikelnumret först och faller tillbaka på söket när det svarar 400, så "Absolut 100" fungerar utan en egen gissning på klienten. **Svaret cachas 30 minuter i Workern** (Cache API, samma `max-age` som Systembolaget själva sätter) så många användare med samma fråga kostar ett anrop. **Svarar Systembolaget 401, 403, 429 eller 5xx söker Workern i spegeln i stället**: via FTS5-indexet `sb_product_fts` (migrering 0009, 2026-09-15) måste varje ord inleda ett ord i namn eller producent (gemener utan diakriter), högst 200 träffar sorteras kortast namn först, tio visas. Indexet är contentless med artikelnumret som rowid och cirka 300 skuggrader, och triggers håller det i fas med `sb_product`. Att fylla det kostar en skriven rad per produkt (27 000 en gång); därefter bara ändrade namn. **`wrangler d1 export` vägrar databaser med virtuella tabeller**: säkerhetskopia före en senare migrering tas med Time Travel (att släppa indexet bryter triggerna). LIKE läste alla 27 035 rader per fråga mot läskvoten. Samma reserv gäller skanningens tre frågor. Produktuppslag på nummer går till spegeln när produktsidan svarar 5xx eller inte nås; en 404 förblir 404.
- **Lager i vald butik** (2026-09-09, backlog P3): `GET /api/stock?drink=&store=` mot `stockbalance/store/{butik}/{productId}`, som ger både saldo och hyllplats ("48 st, hylla 14-04-03"). **Uppslaget sker på Systembolagets interna `productId`, inte artikelnumret**: artikelnumret svarar 200 med `stock: 0` och `isInStoreAssortment: false` på varje butik, alltså tyst fel svar och inte ett fel. Därför bär raden `sb_product_id`, satt vid import, vid nattens uppdatering, och vid första lagerfrågan på en rad från före migreringen. Butiken väljs en gång och sparas i `localStorage` (`flaskor.store`); saldot hämtas på knapptryck i detaljvyn, inte automatiskt per rad, och cachas 10 minuter i Workern per butik och produkt (2026-09-12). Butikslistan (455 butiker) ligger i bundeln, `src/stores.json`, genererad av `npm run stores`. **Önskelistan har samma sak för hela listan**: knappen "Kolla lagret för alla N" hämtar fyra rader i taget via samma route, visar en saldorad med hyllplats per vara och en summering. Resultatet lever i minnet, inte i databasen: saldot åldras på timmar och ska inte se ut som ett faktum efter en omladdning.
...
- **Senast drucken i listan** (2026-09-09, backlog P3): raden i Källaren och Barskåpet visar "Drucken 30 aug 2026 ★★★★★ · 2 ggr" när den druckits, tabellen har kolumnen Drucken (dold från start) och sorteringen nyckeln "Senast drucken". **Flödet är sorteringen, inte en ny vy**: att sortera fallande på senast drucken är precis "vad har vi druckit sist", och kräver varken route, vy eller nav-plats. Hela loggen syns fortfarande bara i detaljvyn; listan får bara senaste raden.
...
- **Backend:** Cloudflare Worker `flaskor-api.buildapp.se` med D1. Hämtar Systembolaget (CORS hindrar webbläsaren), kör cron, cachar sök och lager i Cache API. **Kontot är på Cloudflares gratisplan** (2026-09-12): cirka 2 s CPU per anrop, D1 med 100 000 skrivna och 5 miljoner lästa rader per dygn för hela kontot. Tunga jobb (dumpen) körs därför i GitHub Actions och postas i bitar.
- **Åtkomst** (beslut 2, byggd 2026-09-15): Firebase Auth i projektet `flaskor-d3762`, Google eller e-post med lösenord, SDK:n laddad från Googles CDN som i Beefcake. Klienten skickar ID-token som `Authorization: Bearer`; Workern verifierar den mot Googles nycklar (`jose`), kräver bekräftad e-post och slår upp hushållet i `member`. Tjänstenyckeln `SERVICE_TOKEN` jämförs först och ger tjänsteåtkomst till hushåll 1; `POST /api/refresh-all` och `POST /api/assortment` kräver den och nekar konton. Inloggningen slås på med `apiKey` i `src/config.ts`; tom nyckel ger den gamla grinden. Konton går med i ett hushåll bara via inbjudningskoden under Konto; en sparad grindkod i webbläsaren glöms vid inloggning (grenen borttagen 2026-09-16, `8d3e5dd`).
- **Gästläget** (2026-09-15, Patrik: "man ska kunna leka runt i appen och det ska sparas i localdata"): "Prova utan konto" på inloggningskortet. Rader och avsmakningar ligger i `localStorage` (`flaskor.local`, `src/local.ts`) med samma form som servern svarar med; `StoreProvider` väljer källa (`backend`), resten av appen ser ingen skillnad. Uppslag hos Systembolaget, Vivino och Caviste (`GET /api/systembolaget`, `/api/search`, `/api/caviste`, `/api/vivino`) svarar utan `Authorization`, med `LOOKUP_LIMIT` räknat per IP-adress; allt annat ger 401. **Kräver konto, visas som ett kort med förklaring:** skanning (Geminis gemensamma kvot), lagersaldo (slås upp på serverns rad), uppdatera-knappen och nattens pris och betyg, hushåll och synk. **Påminnelser:** en rad överst i varje vy (kan döljas för sessionen), en rad efter första sparningen och sedan var femte, och Konto-fliken jämför vad som fungerar med och utan konto. **Till konto:** vid inloggning laddas de lokala raderna upp om kontots hushåll är tomt (`POST /api/drinks/import`, högst 40 rader plus avsmakningar per anrop för D1:s tak på 50 frågor, varje bit tas bort lokalt när servern svarat); har kontot redan rader frågar Konto "lägg till eller släng".
- **Exportera data** (2026-09-15): Konto har JSON (hela `ExportData`: rader och avsmakningar, från `GET /api/export` eller lokalt) och CSV (en rad per flaska för svensk Excel: semikolon, decimalkomma, BOM, `src/export.ts`). Ingen import av JSON-filen tillbaka än.
- **Tak per konto:** Rate Limiting-bindningen, `SCAN_LIMIT` 10 per minut (skanning och gå med i hushåll) och `LOOKUP_LIMIT` 60 per minut (allt som hämtar från Systembolaget, Vivino eller Caviste, och nya rader). Grindkoden räknas inte. Geminis dygnskvot delas fortfarande av alla.
- **Integritet:** texten ligger i `S.privacy` (`src/strings.ts`), visas på inloggningen och under Konto. Gemini-anrop från EES följer Googles villkor för betaltjänster även på gratisnivån, alltså ingen träning på bilderna (kollat 2026-09-15).
...
- Systembolaget gav nej till officiell API-åtkomst (2026-09-12), så frontendnyckeln och produktsidan är de enda direkta vägarna, och spegeln (§Flöden) är reserven. Tredjepartsdumpen `susbolaget.emrik.org/v1/products` (C4illin/systembolaget-data, verifierad 2026-09-12): 27 035 produkter, 100 MB rå och 8 MB gzip, `Last-Modified` 03:01 svensk tid, ETag, `Cache-Control: public, max-age=14400`. Bär productNumber, productId, namn, producent, årgång, pris (`price`, inte `priceInclVat`), kategori 1 till 4, ursprung, druvor (lista), alkohol, usage, taste, lagerflaggor och bilder, men inget lagersaldo per butik. Privat drift, kan försvinna: då åldras spegeln efter 36 timmar och allt går på produktsidor igen som före. `AlexGustafsson/systembolaget-api-data` är avlyst (senast uppdaterad 2025-11-02). Systembolagets egen söksida `/sortiment/?q=` renderas i klienten och bär inga träffar i HTML.
- Tredjepartsdumpens `isTemporaryOutOfStock` är `true` på samtliga 26 903 rader medan produktsidan säger `false` för samma vara (kollat 2026-09-13), så spegeln ignorerar den. `isCompletelyOutOfStock` (1 899), `isSupplierTemporaryNotAvailable` (2 930) och `assortment` varierar och stämmer mot produktsidan. `isDiscontinued` är `false` på alla: utgått syns bara som att raden försvinner ur dumpen.
- Nattskriptet postar bara fälten i `DUMP_FIELDS` (`shared/assortment.ts`). Läser Workerns `fromDump` ett nytt fält måste det in i listan, annars blir kolumnen tyst null i hela spegeln (hände 2026-09-13 med `assortment`). Testet kör därför `fromDump(slim(rad))`.
