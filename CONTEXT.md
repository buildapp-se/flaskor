...
| Grindkod | `gate code` | Före inloggningen den delade koden till appen. Nu bara tjänsteåtkomst till hushåll 1 (nattskript, skript); vägen in i hushåll 1 för ett konto togs bort 2026-09-16 när ägarna var inloggade (beslut 2, 10) |
...
- **Åtkomst** (beslut 2, byggd 2026-09-15): Firebase Auth i projektet `flaskor-d3762`, Google eller e-post med lösenord, SDK:n laddad från Googles CDN som i Beefcake. Klienten skickar ID-token som `Authorization: Bearer`; Workern verifierar den mot Googles nycklar (`jose`), kräver bekräftad e-post och slår upp hushållet i `member`. Grindkoden jämförs först och ger tjänsteåtkomst till hushåll 1; `POST /api/refresh-all` och `POST /api/assortment` kräver den och nekar konton. Inloggningen slås på med `apiKey` i `src/config.ts`; tom nyckel ger den gamla grinden. Konton går med i ett hushåll bara via inbjudningskoden under Konto; en sparad grindkod i webbläsaren glöms vid inloggning (grenen borttagen 2026-09-16, `8d3e5dd`).
