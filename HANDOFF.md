...
nextAction: Patrik raderar Workerns gamla GATE_CODE (npx wrangler secret delete GATE_CODE), ger Julia inbjudningskoden under Konto, och säger ja eller nej till att radera det tomma hushållet 2. Det tomma hushållet 2 (Patriks första inloggning) kan raderas för hand.
blockers: []
...
**2026-09-16 kl. 16:10, grindkodsgrenen borta och titeln klickbar** (`8d3e5dd`, Pages via CI, Worker `8c940b75`): `joinHousehold` tar bara inbjudningskod, klientens autoinloggning med sparad grindkod ersatt av `clearGate()`, testet vänt (grindkoden ger 404). Titeln i sidnavigeringen är `<a href="#/">`: ändringen låg ocommittad i arbetsträdet sedan okänd session, därför "fortfarande inte klickbar" live. Verifierat live med Playwright: `<a>` i appen, klick från Konto ger `#/`. **Omdöpt 2026-09-16 kl. 16:30: `GATE_CODE` heter `SERVICE_TOKEN`** (Worker, tester, skript, workflow `FLASKOR_SERVICE_TOKEN`, `.dev.vars`), **roterad kl. 17:40 av Patrik** (`SERVICE_TOKEN` i Workern, `FLASKOR_SERVICE_TOKEN` i Actions, `.dev.vars`, gamla Actions-hemligheten borta), Worker deployad `27a4d7cc`, verifierat live: ny nyckel 200 på `/api/drinks`, fel eller ingen 401. **Kvar:** Workerns gamla `GATE_CODE` ligger oanvänd kvar (`wrangler secret delete` saknar `--force` i 4.132, frågar interaktivt), Patrik kör `npx wrangler secret delete GATE_CODE`. Kommandot som kördes:
...
- **Klient:** `src/local.ts` är en backend med samma metoder som `api`; `StoreProvider` tar `guest`. Kortet `Locked` på skanning, lagersaldo (detaljvyn och Önskelistan) och uppdatera-knappen. Rad överst i varje vy, påminnelse efter sparning 1, 5, 10 och så vidare, Konto-flik för gäst med jämförelse, export och "Rensa lokal data". Vid inloggning laddas lokala rader upp om hushållet är tomt, annars frågar Konto. Export JSON och CSV (semikolon, decimalkomma, BOM) för både gäst och konto.

Verifierat: `npm run check` (tsc, 71 enhetstester varav nya för CSV och uppladdningens bitar, 83 Worker-tester varav nya för uppslag utan konto, tak per IP, import och export, torrdeploy), och i Chromium mot `vite dev` + `wrangler dev`: Prova utan konto, artikelnummer 7562401 slaget utan konto och sparat till källaren, påminnelsen visad, plus och en avsmakning sparade lokalt och kvar efter omladdning, låsen på skanning, lager och uppdatera öppnas på tryck, båda exporterna med rätt innehåll (JSON 1 rad och 1 avsmakning, CSV med BOM som byte EF BB BF), "Logga in eller skapa konto" visar att flaskan följer med, tillbaka till gäst med datan kvar, 390 px. Live: uppslag utan konto 200, `/api/drinks` och `/api/scan` utan konto 401, bundeln bär texterna. **Overifierat:** uppladdningen vid en riktig inloggning (bitningen är enhetstestad, routen Worker-testad).

Tidigare samma dag: **Öppen för fler hushåll** (batch-läge, Patrik: "Jag har tänkt att dela den på ai-forum och i framtiden vinälskar-forum"). Tre commits plus docs, **live 2026-09-15 kl. 14:45 utom själva inloggningen** (den väntar på webbappens nycklar, se §Nästa steg 00):

1. `6e61858` **Worker.** Firebase ID-token verifieras mot Googles nycklar med `jose` (som Beefcake), bekräftad e-post krävs. `member` knyter uid till hushåll; ett nytt konto får ett eget tomt hushåll vid första anropet. `GET /api/me`, `PATCH /api/household`, `POST /api/household/join` (inbjudningskod, eller den gamla grindkoden för hushåll 1; 409 om det egna hushållet har rader), `DELETE /api/me` (hushållet med rader och loggar försvinner när ingen annan är kvar, aldrig hushåll 1). Grindkoden är kvar som tjänsteåtkomst till hushåll 1, och `refresh-all` och `assortment` nekar nu konton (403). Rate Limiting-bindningen: 10 per minut på skanning och gå med, 60 per minut på uppslag och nya rader. Listfrågan och loggaggregatet begränsas till hushållet så sidladdningen inte läser andras rader.
2. `fafc1ee` **Klient.** Inloggningskort (Google först, e-post med registrering, glömt lösenord, bekräftelsevy), kontovy `#/konto` (femte platsen i navigeringen) med hushållsnamn, medlemmar, inbjudningskod, gå med, logga ut, radera konto, integritetstext. **Inloggningen är av tills `apiKey` står i `src/config.ts`**; tom nyckel ger grindkoden som förut, så en push utan nyckel ändrar bara att Konto finns. En webbläsare med sparad grindkod går med i hushåll 1 vid första inloggningen utan klick och glömmer koden. Firebase-SDK:n cachas av service workern, annars startar PWA:n inte utan nät.
3. `62730d7` **FTS5 i spegeln** (migrering 0009). Contentless, `columnsize=0`, `detail=none`, artikelnumret som rowid, triggers för insert, delete och ändrad söktext. Lokalt mot 27 035 rader: cirka 300 skuggrader, "barolo" 386 träffar som LIKE, "glenlivet 12" rätt flaska. Ordstart i stället för delsträng. `upsertAssortment` räknar via `RETURNING` eftersom `meta.changes` nu tar med indexets rader.

Verifierat: `npm run check` (tsc, 65 enhetstester, 79 Worker-tester varav 12 nya i `worker/test/auth.test.ts` med riktigt RS256-signerade token mot en testnyckel, torrdeploy), migrering 0008 och 0009 mot lokal D1 med riktiga data, och i Chromium mot `vite dev` + `wrangler dev`: kontovyn i grindläge på 1 280 och 390 px, och inloggningskortet med en tillfällig nyckel (SDK:n laddades, ingen användare, kortet visades; nyckeln återställd). **I molnet samma eftermiddag** (Patrik: "om du är säker på att det bara kommer att bli 27% så kör"): kostnaden mättes först i molnet med ett tillfälligt index över 1 000 rader (1 000 skrivna, samma som lokalt, indexet borttaget), dygnets skrivningar lästa för alla sju databaser (cirka 1 400), säkerhetskopia `backup-before-0008.sql` i sessionens scratchpad (26 810 rader, 60 drink), `migrations apply --remote` gav 0008 och 0009 ✅. Kontroll: hushåll 1 har inbjudningskod, 3 triggers, "barolo" 380 träffar i indexet, 60 rader kvar. `rows_written_24h` för flaskor gick från 1 025 till 27 781, alltså cirka 26 750 för fyllningen. Worker `20fd7bc3` deployad (Rate Limiting-bindningarna godtogs på gratisplanen), live: `/api/me` med grindkoden ger hushåll 1, `/api/drinks` 60 rader, falsk token 401, sök 10 träffar. Pages-körning 34970846664 grön, bundeln `index-9qwUTQcI.js` bär kontovyn. **Overifierat:** en riktig inloggning, och spegelsöket via Workern live (bara via SQL, eftersom Systembolagets sök svarade).

**Rättat på vägen:** integritetstexten sa först att Google får träna på etikettfotona. Googles villkor säger motsatsen för EES även på gratisnivån (kollat 2026-09-15), texten säger nu det.

Tidigare samma dag: **Minus och plus i tabellens Antal-kolumn** (Patriks P0 under BACKLOG §Captured). `CellarTable` fick propen `onPatch`; skickas den visar Antal `[−] 3 [+]`, samma patch som listans `CountStepper`. Källaren och Barskåpet skickar den, Önskelistan har ingen Antal-kolumn. Radklick ignorerar nu knappar, så minus öppnar inte detaljvyn. Verifierat: `npm run check` (tsc, 65 enhetstester, 67 Worker-tester, torrdeploy) och i Chromium mot `vite dev` + `wrangler dev`: plus gav 3 till 4, minus tillbaka till 3, sparat i lokal D1, adressen oförändrad, radhöjden 49 px; 19 steppers i Barskåpets tabell. Ikonerna: Patrik 2026-09-15, "ikonerna är lösta".

**Nattens spegelkörning 2026-09-14 kl. 08:34 bekräftar D1-fixen (§Nästa steg 0c):** "ok: 26737 rader speglade (2 ändrade, 166 utgångna borttagna) på 33 s". D1-panelens Rows written är inte avläst.

Tidigare: Senast uppdaterad: 2026-09-13. **Tillgängligheten i Önskelistan** (commits `7fa58a7` kod, `7ea35d3` ikoner, pushade; Pages-bygget går på push). Patrik: "Tillfälligt slut" går inte att tolka. Fyndet: alla 19 Systembolagsrader stod som `temporarily_out` eftersom dumpen har `isTemporaryOutOfStock: true` på samtliga 26 903 rader (produktsidan säger `false` för samma vara). Byggt: spegeln ignorerar flaggan, `sb_assortment` på raden, `availability` vidgad med `supplier_out` och `sold_out`, Önskelistan skriver "sortiment · status". Migrering `0007_availability.sql` bygger om `drink` (vidgad CHECK) **och** `tasting`, se §Fällor. Verifierat: `npm run check` (tsc, 65 enhetstester, 66 Worker-tester, torrdeploy) och migreringen mot en lokal D1 med en seedad rad plus avsmakning: båda kvar, inga spår av `drink_old`, båda indexen, kaskadraderingen fungerar. **Klart kl. 17:30 samma dag:** Patrik körde migrering 0007 i molnet (11 kommandon, 60 drink-rader och 0 tasting-rader före och efter, samma som säkerhetskopian), Worker `be3892f7` deployad, spegeln körd om, refresh-all gav `refreshed 19, mirrored 19, failed 0`. **Fälla på vägen:** första körningen gav `sb_assortment` null på alla 19 trots att koden skrev kolumnen. Nattskriptet slimmar varje dumprad till `DUMP_FIELDS` i `shared/assortment.ts` innan den postas, och listan fick aldrig `assortment` och `isSupplierTemporaryNotAvailable` i går, så Workerns `fromDump` såg dem inte. Testet fångade det inte eftersom det matade `fromDump` med råa fält. Fix `c2258fa`: fälten i listan, `slim` flyttad till `shared/`, testet kör `fromDump(slim(rad))` och bevisat att det faller utan fixen. Efter ny spegelkörning: 11 FS, 5 BS i lager, 2 BS slut hos leverantören, 1 TSLS. Live-bundeln `index-BNSW87ti.js` bär texterna; webbläsarkontrollen är Patriks. Säkerhetskopia av molndatabasen före migreringen ligger i sessionens scratchpad (`backup-before-0007.sql`, 26 975 rader).

Tidigare: 2026-09-12. **Spegeln av Systembolagets sortiment** (commits `8890ca7`, `558e67d`, `d344980`, migrering `0006_sb_product.sql` körd i molnet, Worker `4fadb2e4`). Bakgrund: Systembolaget sa nej till officiell API-åtkomst, så de inofficiella vägarna (frontendnyckeln, produktsidan) är de enda, och de ska tåla både att nyckeln dör och många användare. Chunk-läge på "kör".
...
00. **Få inloggningen live (2026-09-15).** I den här ordningen, varje steg förutsätter det förra:
...
0b. **Nattens spegelimport var i drift (2026-09-12 kl. 10:43).** Patrik skapade Actions-hemligheten `FLASKOR_GATE_CODE` och körde workflowen för hand: körning 1 grön på 41 s, loggen "ok: 27035 rader speglade (27035 skickade, 0 gamla borttagna) på 34 s", och `sb_meta.imported_at` i molnet bär körningens stämpel `2026-09-12T08:43:06Z`. Från och med i natt kör den själv 01:30 UTC. Byts grindkoden måste hemligheten bytas med: https://github.com/buildapp-se/flaskor/settings/secrets/actions.
0c. **D1-skrivkvoten sprängd 2026-09-13 kl. 17:28, fix live kl. 18:35.** Kontot fick 100 000 skrivna rader per dygn passerade (162 950): spegeln kördes tre gånger den dagen (08:25 schemalagd, 17:18 och 17:23 för hand efter `DUMP_FIELDS`-fixen) och `INSERT OR REPLACE` kostar två skrivningar per rad (delete plus insert), 3 × 26 903 × 2 = 161 418. Alla D1-skrivningar på kontot (alla sju databaser) gav fel till kl. 02:00 den 14:e; läsning och data opåverkade, spegeln hel. Fix `fd88043`, Worker `d081549e`: upsert med `ON CONFLICT DO UPDATE ... WHERE excluded.json <> sb_product.json` så oförändrade rader inte skrivs alls, och `done` bär alla artikelnummer så utgångna rader hittas som skillnad i JS och raderas i satser om 100 (`updated_at` stämplas inte längre om på oförändrade rader). Bevis: lokalt `changes()` 1 mot 2; Worker-test "samma dump igen kostar noll"; och en handkörning kl. 18:35 mot molnet med kvoten fortfarande spärrad gav 90 × 200 på bitarna och 500 bara på stämpeln i `sb_meta` (observability), alltså skrev bitarna inget. **Kvar att läsa av efter nattens körning:** https://github.com/buildapp-se/flaskor/actions/workflows/assortment.yml (loggen "ok: 26903 rader speglade (N ändrade, M utgångna borttagna)") och https://dash.cloudflare.com/?to=/:account/workers/d1 (Rows written för dygnet). Blir N tiotusentals trots fixen är json-jämförelsen bruten, inte kvoten.
...
- **`wrangler d1 export` vägrar databaser med virtuella tabeller** (Cloudflares dokumentation, 2026-09-15). Efter migrering 0009 tas säkerhetskopian med Time Travel. Släpp inte `sb_product_fts` för att exportera: triggerna pekar på den och varje spegelskrivning skulle ge fel.
- **`meta.changes` räknar trigger-skrivningar** i D1 (4 produkter gav 14 efter 0009). Räkna egna rader med `RETURNING`.
- **Deploya inte Workern före migrering 0008 och 0009**: `/api/me` saknar `invite_code` och spegelsöket saknar `sb_product_fts`, båda ger 500.
- **`ALTER TABLE drink RENAME` skriver om `tasting`'s främmande nyckel** till det nya namnet (SQLite, 2026-09-13). Ombyggnaden i 0003-stil hade lämnat loggen pekande på `drink_old`, och `DROP drink_old` hade kunnat radera den via `ON DELETE CASCADE`. `PRAGMA legacy_alter_table` hjälper inte i D1. Lösningen i 0007: bygg om `tasting` mot nya `drink` innan `drink_old` släpps. Bevisa varje tabellombyggnad lokalt med en seedad rad plus barnrad innan den körs i molnet.
- **`wrangler dev` under Claude Code svarar 401 på allt.** Starta med agentvariablerna avstängda, metoden står i vaultnoten `Browser Automation`. För kontroll i webbläsaren utan att grindkoden hamnar i chatten: `--var SERVICE_TOKEN:test-kod` och skriv `test-kod` i grinden.
- **Lokal D1 kan vara tom** även om migreringarna står som körda (2026-09-06). `npm run seed` fyller på från Excel-raderna.
- **Kontot är på Cloudflares gratisplan** (bekräftat 2026-09-12 av kvotmailet om Durable Objects). Workern dör på fel 1102 när ett anrop drar mer än ungefär 2 s CPU (mätt: 2 020 ms), och D1 har 100 000 skrivna och 5 miljoner lästa rader per dygn för hela kontot. Ingen tung parsning i Workern, och räkna rader lästa innan en fråga får skanna hela `sb_product`. **`INSERT OR REPLACE` kostar två skrivna rader per rad** (delete plus insert): tre fulla speglingar 2026-09-13 stängde D1-skrivningar för hela kontot till midnatt UTC. Skriv bara det som ändrats (upsert med `WHERE`), och en skenande skrivning i ett projekt stänger alla sju databaserna.
