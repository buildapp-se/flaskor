...
schemaVersion: 1
status: active
currentGoal: 2026-09-15 Flaskor öppen för fler hushåll, byggd lokalt i batch-läge och inte i molnet än. Firebase-inloggning (Google, e-post), ett hushåll per konto med inbjudningskod, radera konto, integritetstext, tak per konto, nattjobb och spegelimport bara för grindkoden, FTS5 i spegeln (commits 6e61858, fafc1ee, 62730d7, ej pushade). Före det minus och plus i tabellens Antal-kolumn (5da909c, live) och D1-fixen bekräftad av nattens spegelkörning (2 ändrade, 166 borttagna).
nextAction: Patrik: webbapp i Firebase-projektet flaskor-d3762 och buildapp.se som authorized domain, klistra apiKey och appId till Claude. Ja eller nej på migrering 0009 (cirka 27 000 skrivna D1-rader en gång). Sedan i ordning: export-backup, npm run db:migrate:remote (0008 och 0009), Worker-deploy, push. Se §Nästa steg 00.
blockers: ["Firebase-webbappens apiKey och appId (Patrik)", "ja på migrering 0009:s D1-skrivningar (Patrik)"]
reviewedAt: 2026-09-15
---

# Handoff: Flaskor

Senast uppdaterad: 2026-09-15, andra omgången. **Öppen för fler hushåll** (batch-läge, Patrik: "Jag har tänkt att dela den på ai-forum och i framtiden vinälskar-forum"). Tre lokala commits, **inte pushade och inte i molnet**, eftersom Workern kräver migrering 0008 och 0009 och Pages deployar på push:
...
Verifierat: `npm run check` (tsc, 65 enhetstester, 79 Worker-tester varav 12 nya i `worker/test/auth.test.ts` med riktigt RS256-signerade token mot en testnyckel, torrdeploy), migrering 0008 och 0009 mot lokal D1 med riktiga data, och i Chromium mot `vite dev` + `wrangler dev`: kontovyn i grindläge på 1 280 och 390 px, och inloggningskortet med en tillfällig nyckel (SDK:n laddades, ingen användare, kortet visades; nyckeln återställd). **Overifierat:** en riktig inloggning. Det kräver webbappens nycklar, se §Nästa steg 00.
...
00. **Få inloggningen live (2026-09-15).** I den här ordningen, varje steg förutsätter det förra:
    ...
    3. **Patrik, säkerhetskopia, innan 0009:** `! npx wrangler d1 export flaskor --remote --output backup-before-0008.sql`. Efter 0009 vägrar export (virtuell tabell); då gäller Time Travel.
    4. **Patrik, migreringarna:** läs först dygnets D1-förbrukning (https://dash.cloudflare.com/?to=/:account/workers/d1), sedan `! npm run db:migrate:remote`. 0008 kostar ett par rader, **0009 cirka 27 000 skrivna rader en gång** (mätt: en per produkt). Inte samma dygn som en full spegelkörning.
    5. **Claude:** `npm run worker:deploy`, kontroll av `/api/me` med grindkoden, sedan push så Pages bygger.
