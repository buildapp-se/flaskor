...
schemaVersion: 1
status: active
currentGoal: Tillgängligheten rättad 2026-09-13 (commit 7fa58a7, pushad, migrering 0007 INTE körd i molnet, Workern INTE deployad): dumpens isTemporaryOutOfStock var true på alla rader så allt stod som tillfälligt slut; nu sortimentskod plus fem lägen. Före det spegeln 2026-09-12 (beslut 23), live: D1-tabell med 27 035 rader fylld av GitHub Actions varje natt, reserv för sök och produktuppslag när frontendnyckeln eller produktsidan felar, Cache API på sök och lager, nattens prisuppdatering läser ur spegeln utan produktsidor. Kvar för att natten ska gå av sig själv: hemligheten FLASKOR_GATE_CODE i repots Actions-secrets, som bara Patrik får skapa.
nextAction: Patrik kör `npm run db:migrate:remote` (§Nästa steg 0), sedan deployar Claude Workern, kör spegelworkflowen om och refresh-all, och verifierar att de 19 raderna fått riktiga lägen. Sedan ägar-QA av 2026-09-09 (namnsök, Caviste-länk, Drucket, "Kolla lagret för alla"), de tre Caviste-bildlänkarna, omskanningen. Ja eller nej på FTS5 i spegeln (BACKLOG §Spegeln P2) och på Distiller-importen.
blockers: []
...
0. **Kör migrering 0007 i molnet, sedan säg till.** Klassificeraren stoppade `wrangler d1 migrations apply flaskor --remote` i både Bash och PowerShell 2026-09-13. Kör själv i sessionen:
   ```
   ! npm run db:migrate:remote
   ```
   Därefter gör Claude resten: `npm run worker:deploy`, `gh workflow run assortment.yml` (spegeln får sortimentskoden), `POST /api/refresh-all` med grindkoden ur `.dev.vars`, och en D1-läsning som visar att de 19 raderna inte längre alla är `temporarily_out`. Workern får inte deployas före migreringen: nya koden skriver kolumnen `sb_assortment`.
